...
class SACAgent(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        """
        Initializes the SAC agent with the provided configuration parameters and sets up the neural network models and optimizers.
        
        Parameters:
            base_name (str): The base name for logging and saving models.
            params (dict): Configuration parameters including network details, learning rates, and other hyperparameters.
        """
    ...
    def load_networks(self, params):
        """
        Loads the neural network models as specified in the configuration parameters.
        
        Parameters:
            params (dict): Includes model configuration necessary for initializing the networks.
        """
    ...
    def base_init(self, base_name, config):
        """
        Basic initialization common to different algorithms, setting up environment, number of actors, and other base configurations.
        
        Parameters:
            base_name (str): Base name for the algorithm, used for logging and saving outputs.
            config (dict): Configuration dictionary that includes environmental and training setup.
        """
    ...
    def alpha(self):
        """
        Returns the current value of alpha (temperature parameter) by exponentiating the logarithmic form stored.
    ...
    def device(self):
        """
        Returns the device where tensors should be allocated (CPU or GPU).
        
        Returns:
            torch.device: The device (e.g., 'cuda:0').
        """
        return self._device

    def get_weights(self):
        """
        Retrieves the current weights of the SAC actor and critic networks.
        
        Returns:
            dict: A dictionary containing the weights of the actor, critic, and critic target networks.
        """
    ...
    def save(self, fn):
        """
        Saves the agent's state, including the model weights and optimizers' states to a file.
        
        Parameters:
            fn (str): File name or path where the state should be saved.
        """
    ...
    def set_weights(self, weights):
        """
        Sets the weights of the SAC actor, critic, and critic target networks from a given state dictionary.
        
        Parameters:
            weights (dict): Weights dictionary to load into the networks.
        """
    ...
    def get_full_state_weights(self):
        """
        Gets the complete state of the agent, including model weights, optimizer states, and training counters.
        
        Returns:
            dict: A comprehensive state dictionary of the agent.
        """
    ...
    def set_full_state_weights(self, weights, set_epoch=True):
        """
        Sets the complete state of the agent from a given state dictionary, with the option to update the epoch counter.
        
        Parameters:
            weights (dict): State dictionary containing model weights, optimizer states, and potentially the epoch number.
            set_epoch (bool): Whether to update the epoch counter from the weights.
        """
    ...
    def restore(self, fn, set_epoch=True):
        """
        Restores the agent's state from a file, with the option to update the epoch counter.
        
        Parameters:
            fn (str): File name or path from which to load the state.
            set_epoch (bool): Whether to set the epoch number from the restored state.
        """
        print("SAC restore")
    ...
    def set_param(self, param_name, param_value):
        ...
        Parameters:
            ...
            param_value (any): The value to set for the parameter.
        """
        pass

    def get_masked_action_values(self, obs, action_masks):
        """
        This method is designed to be implemented for environments where action masking is applicable. Raises an error if called without implementation.
        
        Parameters:
            obs (torch.Tensor): Observations from the environment.
            action_masks (torch.Tensor): Masks indicating allowed actions at each state.
        
        Raises:
            NotImplementedError: If called without an implementation.
        """
        assert False

    def set_eval(self):
        """
        Sets the model to evaluation mode, affecting layers like dropout and batch normalization.
        """
        self.model.eval()

    def set_train(self):
        """
        Sets the model to training mode, affecting layers like dropout and batch normalization.
        """
        self.model.train()

    def update_critic(self, obs, action, reward, next_obs, not_done, step):
        """
        Updates the critic model based on the sampled data from the replay buffer.
        
        Parameters:
            obs (torch.Tensor): Observations at the current timestep.
            action (torch.Tensor): Actions taken at the current timestep.
            reward (torch.Tensor): Rewards received after taking actions.
            next_obs (torch.Tensor): Observations at the next timestep.
            not_done (torch.Tensor): Boolean tensor indicating if the episode is continuing.
            step (int): Current step of the optimization.
        
        Returns:
            tuple: Tuple containing detached tensors of the critic loss, first critic component loss, and second critic component loss.
        """
        with torch.no_grad():
    ...
    def update_actor_and_alpha(self, obs, step):
        """
        Updates the actor model and the temperature parameter alpha, based on the sampled data.
        ...
        Returns:
            tuple: Contains detached tensors of the actor loss, entropy, alpha, and alpha loss if temperature is learnable.
        """
    ...
    def soft_update_params(self, net, target_net, tau):
        """
        Soft updates the parameters of a target network based on the corresponding network using the specified tau.
        
        Parameters:
            net (nn.Module): The original network whose parameters to follow.
    ...
    def update(self, step):
        """
        Performs a single update cycle, including sampling from the buffer, updating the critic and actor networks, and soft-updating the target network.
        
        Parameters:
            step (int): The current step number in training used for logging.
        
        Returns:
            tuple: A tuple containing information about the actor and critic updates.
        """
    ...
    def preproc_obs(self, obs):
        """
        Preprocesses the observations using the normalization parameters specified in the model.

        Parameters:
            obs (torch.Tensor or dict): The raw observations from the environment that may need normalization.

        Returns:
            torch.Tensor: The normalized observations ready for processing by the neural network.
        """
    ...
    def cast_obs(self, obs):
        """
        Casts observations to the appropriate torch tensor type based on the data type of the observation space.

        Parameters:
            obs (torch.Tensor or numpy.ndarray): The observations to be cast.

        Returns:
            torch.Tensor: The observations cast to the appropriate tensor type on the correct device.
        """
    ...
    def obs_to_tensors(self, obs):
        """
        Converts observations from various formats to torch tensors, handling dictionaries of observations by recursive processing.

        Parameters:
            obs (dict or array-like): Observations that might be in a dictionary format or direct array-like structures.

        Returns:
            dict or torch.Tensor: Processed observations, structured in a dictionary if provided in such a format.
        """
    ...
    def _obs_to_tensors_internal(self, obs):
        """
        Helper method for recursively processing observations within a dictionary structure to torch tensors.

        Parameters:
            obs (dict or torch.Tensor): Observations in a nested dictionary or direct tensor format.

        Returns:
            dict or torch.Tensor: The recursively processed observation dictionary or torch tensor.
        """
        if isinstance(obs, dict):
            upd_obs = {}
            for key, value in obs.items():
    ...
    def preprocess_actions(self, actions):
        """
        Converts actions from torch tensors to numpy arrays if not operating directly with tensor observations.

        Parameters:
            actions (torch.Tensor): Actions in torch tensor format.

        Returns:
            numpy.ndarray or torch.Tensor: Actions in numpy array format if required, or original tensor if no conversion is necessary.
        """
    ...
    def env_step(self, actions):
        """
        Takes an action in the environment and observes the result, converting all results to torch tensors.

        Parameters:
            actions (array-like or torch.Tensor): Actions to execute in the environment.

        Returns:
            tuple: A tuple containing the new observations, rewards, dones (booleans indicating the end of an episode), and additional info from the environment.
        """
    ...
    def env_reset(self):
        """
        Resets the environment to start a new episode and preprocesses the initial observations.

        Returns:
            torch.Tensor or dict: The initial state observations from the environment after reset.
        """
        with torch.no_grad():
    ...
    def act(self, obs, action_dim, sample=False):
        """
        Decides on an action based on the current observations using the policy defined by the actor model. Can return either deterministic or sampled actions.

        Parameters:
            obs (torch.Tensor): Current observations.
            action_dim (int): Dimension of the action space.
            sample (bool): Whether to sample from the action distribution (True) or take the mean action (False).
    ...
    def extract_actor_stats(self, actor_losses, entropies, alphas, alpha_losses, actor_loss_info):
        """
        Extracts and logs statistics for the actor's performance from the current update cycle.

        Parameters:
            actor_losses (list): List to append the current actor loss.
            entropies (list): List to append the current entropy value.
            alphas (list): List to append the current alpha value.
            alpha_losses (list): List to append the current alpha loss, if applicable.
            actor_loss_info (tuple): Current actor loss information tuple containing actor loss, entropy, alpha, and alpha loss.
        """
    ...
    def clear_stats(self):
        """
        Clears the accumulated statistics for rewards and episode lengths, and performs additional cleanup through the observer.
        """
    ...
    def play_steps(self, random_exploration = False):
        """
        Executes a series of steps in the environment using either the policy or random actions. Collects and processes data for training.

        Parameters:
            random_exploration (bool): If True, actions are chosen randomly; otherwise, the policy is used to determine actions.

        Returns:
            tuple: Aggregated statistics about the steps played including total and step times, losses, and actor/critic updates.
        """
    ...
    def train_epoch(self):
        """
        Executes one training epoch which includes playing through a set number of steps and performing updates based on the interactions.

        Returns:
            tuple: Results from the play steps, including timing and loss information.
        """
    ...
    def train(self):
        """
        Handles the complete training loop until a stopping criterion is met, such as reaching a maximum number of frames or epochs.

        Returns:
            tuple: Final mean rewards and the number of epochs completed.
        """
        self.init_tensors()
